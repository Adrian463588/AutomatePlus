...
- Missing or unverified packs, devices, target apps, credentials, selectors, or
  ...
  a simulated pass.
- Browser mode shows runtime catalog status as `Unavailable` until the native
  host scans local evidence; it must not represent an unevaluated catalog as
  zero installed packs.
