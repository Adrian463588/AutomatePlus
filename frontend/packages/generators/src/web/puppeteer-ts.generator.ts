...
export class PuppeteerTsGenerator extends BaseCodeGenerator {
  ...
  public generateStep(action: ActionIR, _session: SessionIR, _options?: GeneratorOptions): string {
    ...
    switch (action.action) {
      ...
      case 'assertVisible':
        return `    const isVisible = await page.$eval('${selector}', (el: any) => !!(el.offsetWidth || el.offsetHeight || el.getClientRects().length));\n    expect(isVisible).toBe(true);`;

      case 'assertText':
        return `    const text = await page.$eval('${selector}', (el: any) => el.textContent);\n    expect(text).toContain(${expected});`;

      case 'assertValue':
        return `    const val = await page.$eval('${selector}', (el: any) => el.value);\n    expect(val).toBe(${expected});`;
