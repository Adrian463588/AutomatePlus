...
export const ACTIONS_BY_PLATFORM: Record<PlatformType, readonly ActionType[]> = {
  web: WEB_ACTIONS,
...
type ActionIRLike = {
  ...
  action: ActionType;
  locators?: Array<z.infer<typeof LocatorCandidateSchema>>;
  value?: SecretValue;
  attributeName?: string;
  expectedValue?: string;
  dragTarget?: z.infer<typeof DragTargetSchema>;
  scrollOffset?: z.infer<typeof ScrollOffsetSchema>;
  swipeVector?: z.infer<typeof SwipeVectorSchema>;
  coordinates?: z.infer<typeof CoordinateSchema>;
  apiPayload?: z.infer<typeof ApiPayloadSchema>;
  assertion?: Assertion;
};

const LOCATOR_ACTIONS = new Set<ActionType>([
...
function hasLocator(action: ActionIRLike): boolean {
  return Boolean(action.locators && action.locators.length > 0);
}

function hasExpectedValue(action: ActionIRLike): boolean {
  return action.expectedValue !== undefined || action.value !== undefined || action.assertion?.expected !== undefined;
}

function expectedNumber(action: ActionIRLike): number | undefined {
...
function validateActionCompatibility(action: ActionIRLike, ctx: z.RefinementCtx): void {
  ...
  if (action.locators) {
    for (const error of validateLocatorRanking(action.locators)) addIssue(ctx, ['locators'], error);
  }
  if (LOCATOR_ACTIONS.has(action.action) && !hasLocator(action)) {
    ...
    if (!coordinateAllowed) addIssue(ctx, ['locators'], `action '${action.action}' requires at least one locator`);
  }

  if (action.platform !== 'android' && action.coordinates !== undefined) addIssue(ctx, ['coordinates'], 'coordinates are only valid for android actions');
  ...
  if (action.apiPayload !== undefined && action.action !== 'httpRequest') addIssue(ctx, ['apiPayload'], "apiPayload is only valid for 'httpRequest'");
  if (action.action === 'httpRequest' && action.apiPayload === undefined) addIssue(ctx, ['apiPayload'], "apiPayload is required for 'httpRequest'");

  if (action.dragTarget !== undefined && !['drag', 'dragAndDrop'].includes(action.action)) addIssue(ctx, ['dragTarget'], `dragTarget is not valid for action '${action.action}'`);
  if (['drag', 'dragAndDrop'].includes(action.action) && action.dragTarget === undefined) addIssue(ctx, ['dragTarget'], `action '${action.action}' requires dragTarget`);

  if (action.scrollOffset !== undefined && action.action !== 'scroll') addIssue(ctx, ['scrollOffset'], "scrollOffset is only valid for 'scroll'");
  if (action.action === 'scroll' && action.scrollOffset === undefined) addIssue(ctx, ['scrollOffset'], "scrollOffset is required for 'scroll'");
  if (action.swipeVector !== undefined && action.action !== 'swipe') addIssue(ctx, ['swipeVector'], "swipeVector is only valid for 'swipe'");
  if (action.action === 'swipe' && action.swipeVector === undefined) addIssue(ctx, ['swipeVector'], "swipeVector is required for 'swipe'");

  switch (action.action) {
    case 'navigate':
      if (typeof action.value !== 'string' || action.value.trim() === '') addIssue(ctx, ['value'], 'navigate requires a non-empty URL string value');
      break;
    case 'fill':
      if (action.value === undefined) addIssue(ctx, ['value'], 'fill requires value');
      break;
    case 'pressKey':
      if (typeof action.value !== 'string' || action.value.trim() === '') addIssue(ctx, ['value'], 'pressKey requires a key value');
      break;
    case 'launchApp':
      if (typeof action.value !== 'string' || action.value.trim() === '') addIssue(ctx, ['value'], 'launchApp requires an app identifier');
      break;
    case 'waitFor':
      if (!hasLocator(action) && typeof action.value !== 'string') addIssue(ctx, ['value'], 'waitFor requires a locator or value');
      break;
    case 'assertVisible':
    case 'assertHidden':
      if (hasExpectedValue(action)) addIssue(ctx, ['expectedValue'], `${action.action} does not accept an expected value`);
      break;
    case 'assertText':
    case 'assertValue':
      if (!hasExpectedValue(action)) addIssue(ctx, ['expectedValue'], `${action.action} requires an expected value`);
      break;
    case 'assertAttribute':
      if (!action.attributeName) addIssue(ctx, ['attributeName'], 'assertAttribute requires attributeName');
      if (!hasExpectedValue(action)) addIssue(ctx, ['expectedValue'], 'assertAttribute requires an expected value');
      break;
    case 'assertUrl':
      if (!hasExpectedValue(action)) addIssue(ctx, ['expectedValue'], 'assertUrl requires an expected URL value');
      if (action.value !== undefined && typeof action.value !== 'string') addIssue(ctx, ['value'], 'assertUrl cannot use a secret reference as its expected URL');
      break;
    case 'assertStatusCode': {
      const status = expectedNumber(action);
      if (status === undefined || !Number.isInteger(status) || status < 100 || status > 599) addIssue(ctx, ['expectedValue'], 'assertStatusCode requires an integer HTTP status between 100 and 599');
      break;
    }
    case 'assertJsonPath':
      if (!action.attributeName && !action.assertion?.jsonPath) addIssue(ctx, ['attributeName'], 'assertJsonPath requires a JSONPath');
      if (!hasExpectedValue(action)) addIssue(ctx, ['expectedValue'], 'assertJsonPath requires an expected value');
      break;
    case 'assertHeader':
      if (!action.attributeName && !action.assertion?.headerName) addIssue(ctx, ['attributeName'], 'assertHeader requires a header name');
      if (!hasExpectedValue(action)) addIssue(ctx, ['expectedValue'], 'assertHeader requires an expected value');
      break;
    case 'assertResponseTime': {
      const duration = expectedNumber(action);
      if (duration === undefined || duration <= 0) addIssue(ctx, ['expectedValue'], 'assertResponseTime requires a positive duration in milliseconds');
      break;
    }
    default:
      break;
  ...
  if (action.assertion && !isAssertionAction(action.action)) addIssue(ctx, ['assertion'], `assertion metadata is not valid for action '${action.action}'`);
  if (action.expectedValue !== undefined && !isAssertionAction(action.action)) addIssue(ctx, ['expectedValue'], 'expectedValue is only valid for assertion actions');
  if (action.attributeName !== undefined && !['assertAttribute', 'assertJsonPath', 'assertHeader'].includes(action.action)) addIssue(ctx, ['attributeName'], `attributeName is not valid for action '${action.action}'`);
}
...
export function isAssertionAction(action: ActionType): action is AssertionActionType {
  return (ASSERTION_ACTIONS as readonly ActionType[]).includes(action);
}

export function isActionAllowedOnPlatform(platform: PlatformType, action: ActionType): boolean {
