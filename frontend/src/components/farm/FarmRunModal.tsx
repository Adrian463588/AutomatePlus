...
export const FarmRunModal: React.FC<FarmRunModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4" role="presentation">
      <div ref={dialogRef} className="ap-dialog-elevation flex max-h-[90vh] w-full max-w-2xl flex-col overflow-hidden rounded-lg border border-slate-700 bg-slate-900" role="dialog" aria-modal="true" aria-labelledby="farm-run-title">
        {/* Header */}
        <div className="flex min-h-14 items-center justify-between border-b border-slate-800 bg-slate-950 px-5">
          ...
          <button
            type="button"
            onClick={onClose}
            className="min-h-12 min-w-12 rounded text-slate-400 transition-colors hover:bg-slate-800 hover:text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400"
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="text-slate-300 font-semibold block mb-1.5 flex items-center gap-1.5">
                <Layers className="w-3.5 h-3.5 text-cyan-400" /> Execution Strategy
              </label>
              <select
                value={strategy}
                onChange={(e) => setStrategy(e.target.value as DeviceExecutionStrategy)}
                className="w-full bg-slate-950 border border-slate-700 rounded-md px-3 py-2 text-xs font-mono text-white focus:outline-none focus:border-cyan-500 cursor-pointer"
              >
        ...
        <div className="flex min-h-16 flex-wrap items-center justify-between gap-2 border-t border-slate-800 bg-slate-950 px-5">
          ...
          <button
            type="button"
            onClick={onClose}
            className="min-h-12 rounded px-3 text-xs text-slate-400 transition-colors hover:bg-slate-800 hover:text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400"
          >
            Close
          </button>
          <button
            ...
            disabled={!canLaunch}
            className="flex min-h-12 items-center gap-2 rounded-md bg-emerald-600 px-5 text-xs font-bold text-white transition-colors hover:bg-emerald-500 disabled:cursor-not-allowed disabled:opacity-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400"
            title={launchReason}
          >
