      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-5">
        ...
        <section aria-labelledby="runtime-preflight-heading">
          ...
          <div className="rounded-md border border-slate-800 bg-slate-950/60 p-3 space-y-2">
            <div className="flex items-start gap-2 text-slate-500 leading-5" role="status" aria-live="polite" aria-busy={runtimePreflight.status === 'busy'}>
              <span className={`mt-1 h-2 w-2 rounded shrink-0 ${runtimePreflight.status === 'ready' ? 'bg-emerald-400' : runtimePreflight.status === 'busy' ? 'bg-sky-400' : runtimePreflight.status === 'error' ? 'bg-rose-400' : 'bg-amber-400'}`} />
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <button type="button" onClick={() => setActiveTab('runtime')} title="Open Runtime Manager; license acceptance and native verification are required before downloading missing packs." className="flex min-h-12 items-center gap-1.5 px-2 py-1.5 text-cyan-200 hover:text-white hover:bg-cyan-900/50 rounded border border-cyan-700/70">
        ...
        <section aria-labelledby="sessions-heading">
          ...
          {sessions.length === 0 ? <p className="rounded-md border border-dashed border-slate-800 p-3 text-slate-500 leading-5">No session exists. Create one, then provide its real target.</p> :
            <div className="space-y-1">{sessions.map((session) => <button key={session.id} type="button" onClick={() => void selectSession(session.id)} aria-pressed={activeSession?.id === session.id} className={`w-full flex items-center justify-between p-2.5 rounded-md text-left border ${activeSession?.id === session.id ? 'bg-cyan-600/20 text-cyan-200 border-cyan-500/40' : 'text-slate-300 hover:bg-slate-800/60 border-transparent'}`}>
        ...
        <section aria-labelledby="devices-heading">
          ...
          <div className="rounded-md border border-slate-800 bg-slate-950/60 p-3 space-y-2">
            <div className="flex items-start gap-2 text-slate-500 leading-5"><span className="mt-1 h-2 w-2 rounded bg-slate-600 shrink-0" /> <span>{deviceDiscoveryMessage}</span></div>
