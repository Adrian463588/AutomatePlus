...
import { ActionToolbar, EmptyState } from '../common/UiPrimitives.js';
...
function ProgressIndicator({ progress }: { progress: NonNullable<RuntimePackView['progress']> }): React.ReactNode {
  if (progress.totalBytes === undefined) {
    return (
      <div className="mt-3 rounded border border-cyan-800/70 bg-cyan-950/30 px-3 py-2 text-[11px] text-cyan-200" role="status" aria-live="polite">
  ...
  return (
    <div className="mt-3" role="group" aria-label="Runtime download progress">
      ...
      <div className="h-2 overflow-hidden rounded bg-slate-800" role="progressbar" aria-valuemin={0} aria-valuemax={progress.totalBytes} aria-valuenow={progress.downloadedBytes} aria-label="Runtime download progress">
        <div className="h-full rounded bg-cyan-500 transition-[width] duration-150 motion-reduce:transition-none" style={{ width: `${percentage}%` }} />
      </div>
    </div>
  );
}
    ...
    <main className="flex min-h-0 min-w-0 flex-1 flex-col overflow-x-hidden overflow-y-auto bg-slate-950/60 text-xs text-slate-200" aria-labelledby="runtime-manager-heading" aria-busy={operationBusy}>
      <header className="flex min-w-0 flex-wrap items-start justify-between gap-3 border-b border-slate-800 bg-slate-900 px-4 py-4">
        <div className="flex min-w-0 items-start gap-3">
          <PackageCheck className="mt-0.5 h-6 w-6 shrink-0 text-cyan-400" aria-hidden="true" />
      ...
      <section className="grid min-w-0 gap-3 border-b border-slate-800 bg-slate-900/70 p-4 sm:grid-cols-2 xl:grid-cols-4" aria-label="Runtime summary">
        ...
        <div className="min-w-0 rounded border border-slate-800 bg-slate-950/80 p-3">
          ...
          <p className="mt-2 text-sm font-semibold text-cyan-200">Explicit action only</p>
          <p className="mt-1 text-[10px] text-slate-500">No startup download or cloud fallback.</p>
        </div>
      </section>

      <ActionToolbar aria-label="Runtime actions">
      ...
      {(statusMessage || actionMessage || actionError || reviewPacks.length > 0) && (
        <section className="space-y-2 border-b border-slate-800 bg-slate-950/80 px-4 py-3" aria-label="Runtime status messages">
          {(statusMessage || actionMessage) && <p className="flex items-start gap-2 text-[11px] leading-5 text-slate-300" role="status" aria-live="polite"><Info className="mt-0.5 h-4 w-4 shrink-0 text-cyan-300" aria-hidden="true" />{statusMessage || actionMessage}</p>}
      ...
      {activeJob && (
        <section className="border-b border-slate-800 bg-cyan-950/20 px-4 py-3" aria-label="Active runtime job" role="status" aria-live="polite">
          <div className="flex min-w-0 flex-wrap items-center justify-between gap-2">
            <div className="flex min-w-0 items-center gap-2"><LoaderCircle className={`h-4 w-4 shrink-0 text-cyan-300 ${ACTIVE_JOB_STATUSES.includes(activeJob.status) ? 'animate-spin motion-reduce:animate-none' : ''}`} aria-hidden="true" /><span className="font-semibold text-cyan-100">Native job {activeJob.jobId}</span></div>
            <span className={`rounded border px-2 py-1 text-[10px] font-semibold ${statusClasses(activeJob.status)}`}>{statusLabel(activeJob.status)}</span>
          </div>
      ...
      <section className="min-w-0 flex-1 p-4" aria-labelledby="runtime-pack-list-heading">
        <div className="mb-3 flex min-w-0 flex-wrap items-center justify-between gap-2">
          <div className="flex min-w-0 items-center gap-2"><PackageCheck className="h-5 w-5 shrink-0 text-cyan-400" aria-hidden="true" /><h2 id="runtime-pack-list-heading" className="truncate text-sm font-semibold text-slate-200">Runtime catalog</h2></div>
          <span className="text-[10px] text-slate-400">{catalogStatusLabel}</span>
        </div>
        {packs.length === 0 ? (
          <EmptyState
            title={host.mode === 'browser' ? 'Native runtime catalog unavailable' : 'No runtime catalog entries loaded'}
            description={host.mode === 'browser' ? 'Run AutomatePlus.exe to scan local roots, verify packs, and use the Windows folder picker. Browser mode does not inspect local files.' : 'Run Check runtime or Scan local to load the bundled pinned catalog. Empty state does not imply runtimes are installed.'}
          />
        ) : (
          <div className="runtime-pack-table-wrap">
            <table className="runtime-pack-table text-[11px]" aria-label="Runtime packs">
              <thead>
                <tr>
                  <th scope="col">Pack</th>
                  <th scope="col">Status</th>
                  <th scope="col">Source / license</th>
              ...
              <tbody>
                {packs.map((pack) => (
                  <tr key={`${pack.entry.id}@${pack.entry.version}`}>
                    <td data-label="Pack">
                      <div className="min-w-0">
                        <h3 className="break-words font-semibold text-white">{pack.entry.id}</h3>
                        <p className="mt-1 break-words font-mono text-[10px] text-slate-400">{pack.entry.category} · v{pack.entry.version ?? 'unresolved'} · {pack.entry.architecture}</p>
                      </div>
                    </td>
                    <td data-label="Status">
                      <span className={`inline-flex min-h-8 items-center gap-1.5 rounded border px-2 py-1 text-[10px] font-semibold ${statusClasses(pack.status)}`} aria-label={`Status ${statusLabel(pack.status)}`}>{statusIcon(pack.status)}{statusLabel(pack.status)}</span>
                    </td>
                    <td data-label="Source / license">
                      <p className="break-all font-mono text-slate-300">{sourceHost(pack.entry)}</p>
                      <p className="mt-1 break-words text-slate-400">{licenseLabel(pack.entry)}</p>
                    </td>
                    <td data-label="Integrity">
                      <p className="break-all font-mono text-slate-300">{sourceDigest(pack.entry)}</p>
                      <p className="mt-1 font-mono text-slate-400">{sourceSize(pack.entry)}</p>
                    </td>
                    <td data-label="Evidence">
                      {(pack.reason ?? pack.entry.reviewReason) && <p className="break-words leading-5 text-slate-300">{pack.reason ?? pack.entry.reviewReason}</p>}
                      {pack.progress && <ProgressIndicator progress={pack.progress} />}
                      <p className="mt-2 flex min-w-0 items-start gap-2 text-[10px] leading-4 text-slate-400"><ExternalLink className="mt-0.5 h-3.5 w-3.5 shrink-0" aria-hidden="true" /><span className="break-all">{sourceReference(pack.entry)}</span></p>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="dialog-backdrop" role="presentation">
          <div ref={dialogRef} className="dialog-card" role="dialog" aria-modal="true" aria-labelledby="runtime-license-heading" aria-describedby="runtime-license-description">
            <div className="flex items-start justify-between gap-3">
              <div className="flex min-w-0 items-start gap-2"><ShieldCheck className="mt-0.5 h-5 w-5 shrink-0 text-cyan-300" aria-hidden="true" /><div className="min-w-0"><h2 id="runtime-license-heading" className="text-sm font-bold text-white">Accept runtime licenses</h2><p id="runtime-license-description" className="mt-1 text-[11px] leading-5 text-slate-400">Native host will validate SHA-256, license metadata, executable paths, and health before marking a pack ready.</p></div></div>
              <button type="button" onClick={closeLicenseDialog} className="min-h-12 min-w-12 rounded text-slate-400 hover:bg-slate-800 hover:text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400" aria-label="Close license dialog" title="Close license dialog"><X className="mx-auto h-4 w-4" aria-hidden="true" /></button>
            </div>
            ...
            <label className="mt-4 flex min-h-12 items-start gap-3 rounded border border-slate-800 bg-slate-950 p-3 text-[11px] leading-5 text-slate-200">
              <input type="checkbox" checked={licenseAccepted} onChange={(event) => setLicenseAccepted(event.target.checked)} className="mt-1 h-5 w-5 shrink-0 accent-cyan-500" />
