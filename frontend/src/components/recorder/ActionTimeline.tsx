...
export const ActionTimeline: React.FC = () => {
  ...
  const getActionIcon = (action: ActionType) => {
    switch (action) {
      ...
      case 'hover':
      case 'dragAndDrop':
        return <MousePointer className="w-3.5 h-3.5 text-cyan-400" />;
      case 'tap':
      case 'doubleTap':
      case 'swipe':
      case 'drag':
        ...
        {steps.length === 0 ? (
          ...
          steps.map((step, index) => {
            ...
            return (
              <div
                key={step.id}
                className="bg-slate-900/90 border border-slate-800 hover:border-slate-700 rounded-lg p-2.5 flex items-center justify-between gap-3 group transition-colors"
              >
                  ...
                  <div>
                    ...
                    {primaryLoc && (
                      <div className="flex items-center gap-1.5 mt-0.5">
                        <span className="text-[10px] font-mono bg-cyan-950/80 text-cyan-300 border border-cyan-800/60 px-1.5 rounded">
                ...
                <div className="flex-1 flex items-center gap-2 max-w-xs">
                  {step.value !== undefined && (
                    <input
                      ...
                      className="bg-slate-950 border border-slate-800 rounded px-2 py-1 text-xs text-slate-200 font-mono w-full focus:outline-none focus:border-cyan-500 disabled:opacity-60"
